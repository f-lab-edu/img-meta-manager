...
public class NumberNeCommand implements SearchCommand {
    @Override
    public BooleanExpression execute(SearchFilter searchFilter) throws InvalidSearchException {
        return imageMetaData.key.eq(searchFilter.getKey()).and(imageMetaData.numberValue.ne(searchFilter.getValueToLong()));
