...
public class DateBeforeCommand implements SearchCommand {
    @Override
    public BooleanExpression execute(SearchFilter searchFilter) throws InvalidSearchException {
        return imageMetaData.key.eq(searchFilter.getKey()).and(imageMetaData.dateValue.before(searchFilter.getValueToDate()));
