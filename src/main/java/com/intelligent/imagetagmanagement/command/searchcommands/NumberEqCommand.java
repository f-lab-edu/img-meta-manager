...
public class NumberEqCommand implements SearchCommand {
    @Override
    public BooleanExpression execute(SearchFilter searchFilter) throws InvalidSearchException {
        return imageMetaData.key.eq(searchFilter.getKey()).and(imageMetaData.numberValue.eq(searchFilter.getValueToLong()));
